...
def get_data(objednavatel: str, dict_cena: dict, dict_key: str, db_conn, mail_domain_extension: str = '.sk',
             last_update_str: str = None) -> None:
    '''
    The function is designed to download and process data from emails for direct orders made by various
    hospitals.
    :param objednavatel: A string that represents the name of the hospital whose data is being processed
    :param dict_cena: A dictionary that is used to replace certain values in the price column of the data
    :param dict_key: the key to be used in the dict_all
    :param mail_domain_extension: ): domain extension for the email addresses of the hospitals
    :param last_update_str: last date and time when the data was updated.
    :return:
    '''
    try:
        global df_orig
        ...
        try:
            ...
            db_conn.insert_table(table_name='priame_objednavky', df=obj.df_all)
...
if __name__ == '__main__':
    ...
    fntn.clean_data(fntn_data_cleaning, key='fakultna nemocnica trencin',  db_con_cloud=db_cloud)
    ...
    donsp.clean_data(donsp_table_cleaning, key='dolnooravska nemocnica s poliklinikou mudr l nadasi jegeho dolny kubin',
                        set_column_names=True, db_con_cloud=db_cloud,
